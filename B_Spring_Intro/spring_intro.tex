\section{Introduzione a Spring AI} % (fold)
\label{sec:spring-intro}

\begin{frame}
TBA
\end{frame}

\section{\faWrench\ Inizializzazione \textit{stub} Spring AI} % (fold)
\label{sec:spring-ai-intro}

\begin{frame}[t,fragile] \frametitle{Progetto Spring AI}
    \framesubtitle{Inizializzazione \textit{stub} progetto}
    \begin{itemize}[leftmargin=10pt,align=right]
        \onslide<1->\item[\alert{\faHandORight}] Inizializzare un progetto Spring con le seguenti caratteristiche:     
        \onslide<2->
        \begin{itemize}[leftmargin=10pt,align=right]
            \item[\alert{\faHandORight}] \alert{Maven} come \textit{build tool}
            \item[\alert{\faHandORight}] Spring Boot alla versione più recente \alert{non SNAPSHOT}
            \item[\alert{\faHandORight}] Linguaggio \alert{Java 21}
            \item[\alert{\faHandORight}] \textit{Group} \alert{\texttt{it.venis.ai.spring}}
            \item[\alert{\faHandORight}] \textit{Artifact} \alert{\texttt{demo}}
            \item[\alert{\faHandORight}] \alert{\texttt{jar}} \textit{packaging}
            \item[\alert{\faHandORight}] \alert{\texttt{Spring Web}} e \alert{\texttt{OpenAI}} come dipendenze
        \end{itemize}
    \end{itemize}
\end{frame}

\begin{frame}[t,fragile] \frametitle{Progetto Spring AI}
    \framesubtitle{Strategie di inizializzazione}
	\vspace*{-.5cm}
    \begin{itemize}
        \onslide<1->{\item[\alertedcircled{1}] Da IDE}
        \onslide<2->{\item[\alertedcircled{2}] Da Spring Initializr}
    \end{itemize}
\end{frame}

\begin{frame}[t,fragile] \frametitle{Progetto Spring AI}
    \framesubtitle{1 - Inizializzazione da IDE (VSCode)}
	\vspace*{-.5cm}
    {\footnotesize
    \begin{itemize}
        \only<1|handout:1>{\item[\alertedcircled{1}] Installare le estensioni VSC necessarie}
        \only<2|handout:2>{\item[\alertedcircled{2}] Aprire una nuova finestra VSC e selezionare \textbf{Create Java Project}}
        \only<3|handout:3>{\item[\alertedcircled{3}] Selezionare \textbf{Spring Boot} come tipo di progetto (\alert{NON} Maven)}
        \only<4|handout:4>{\item[\alertedcircled{4}] Selezionare \textbf{Maven project} come tipo di progetto}
        \only<5|handout:5>{\item[\alertedcircled{5}] Selezionare la versione di Boot per il progetto}
        \only<6|handout:6>{\item[\alertedcircled{6}] Selezionare \textbf{Java} come linguaggio di progetto}
        \only<7|handout:7>{\item[\alertedcircled{7}] Definire il \textit{Group ID} di riferimento del progetto}
        \only<8|handout:8>{\item[\alertedcircled{8}] Definire l'\textit{Artifact ID} di riferimento del progetto}
        \only<9|handout:9>{\item[\alertedcircled{9}] Definire la tipologia di \textit{packaging} del progetto}
        \only<10|handout:10>{\item[\alertedcircled{10}] Selezionare la versione di Java di riferimento}
        \only<11|handout:11>{\item[\alertedcircled{11}] Selezionare come prima dipendenza di progetto \textbf{Spring Web}}
        \only<12|handout:12>{\item[\alertedcircled{12}] Selezionare come seconda dipendenza di progetto \textbf{OpenAI}}
        \only<13|handout:13>{\item[\alertedcircled{13}] Continuare la creazione del progetto premendo \textbf{Enter}}
        \only<14|handout:14>{\item[\alertedcircled{14}] Navigare nel FS e selezionare la cartella padre del progetto}
        \only<15|handout:15>{\item[\alertedcircled{15}] Verificare la presenza dei file di progetto nel menu \textbf{Explorer}}
    \end{itemize}
    \vfill
    \begin{minipage}[t]{\textwidth}
		\centering
        \only<1|handout:1>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-extensions.png}
		    \end{figure}
        }
         \only<2|handout:2>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new.png}
		    \end{figure}
        }
        \only<3|handout:3>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-spring-boot.png}
		    \end{figure}
        }
        \only<4|handout:4>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-maven.png}
		    \end{figure}
        }
        \only<5|handout:5>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-spring-boot-version.png}
		    \end{figure}
        }
        \only<6|handout:6>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-java.png}
		    \end{figure}
        }
        \only<7|handout:7>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-maven-groupid.png}
		    \end{figure}
        }
        \only<8|handout:8>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-maven-artifactid.png}
		    \end{figure}
        }
        \only<9|handout:9>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-java-jar.png}
		    \end{figure}
        }
        \only<10|handout:10>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-java-version.png}
		    \end{figure}
        }
        \only<11|handout:11>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-maven-deps-spring-web.png}
		    \end{figure}
        }
        \only<12|handout:12>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-maven-deps-openai.png}
		    \end{figure}
        }
        \only<13|handout:13>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-maven-deps-done.png}
		    \end{figure}
        }
        \only<14|handout:14>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-new-path.png}
		    \end{figure}
        }
        \only<15|handout:15>{
		    \begin{figure}[ht]
			    \includegraphics[width=\textwidth, frame]{img/spring-vsc-demo-project-done.png}
		    \end{figure}
        }  
	\end{minipage}
    }
\end{frame}

\begin{frame}[t,fragile] \frametitle{Progetto Spring}
    \framesubtitle{2 - Inizializzazione da Spring Initializr}
    \vspace*{-.5cm}
    \centering
    \includegraphics[width=.9\textwidth, frame]{img/spring-initializr.png}
    \vfill
    \onslide<2->
    \begin{block}{Portale ufficiale}
        \url{https://start.spring.io/}
    \end{block}
\end{frame}