...
\begin{frame}[t,fragile] \frametitle{Bag-of-Words: come funziona}
	{\footnotesize
		...
		\begin{table}
			...
			\hskip-2.0cm\begin{tabular}{c|c|c|c|c|c|c|c|c|c|c|c}
				%\toprule
		...
		\begin{table}
			\setlength{\tabcolsep}{4pt}
			\renewcommand{\arraystretch}{1}
			\hskip-1.5cm\begin{tabular}{cc|c|c|c|c|c|c|c|c|c|c}
								& \multicolumn{11}{c}{\textbf{Vocabolario}}\\
				\cmidrule{2-12}
				        & che & bel & cane & avere & ma & il & mio & gatto & essere & più & bello\\
				\cmidrule{2-12}
...
\begin{frame}[t,fragile] \frametitle{Bag-of-Words}
	{\small
		...
		\begin{itemize}[leftmargin=10pt,align=right]
			\onslide<1->{\item[\alert{\faHandORight}] \alert{Perdita dell'ordine:} frasi diammetralmente opposte hanno la medesima rappresentazione}
			\onslide<2->{\item[\alert{\faHandORight}] \alert{Mancanza di semantica:} Non cattura il significato delle parole
			\item[\alert{\faHandORight}] \alert{Nessuna generalizzazione:} i sinonimi sono trattati come elementi \alert{totalmente separati}}
			\onslide<3->{\item[\alert{\faHandORight}] \alert{Alta dimensionalità:} Vocabolari enormi con molti zeri}
		...
		\begin{minipage}[t]{.75\textwidth}
			...
			\setlength{\beforeepigraphskip}{0pt}
			\setlength{\epigraphwidth}{0.9\textwidth}
			\epigraph{\textit{\alert{F1:} ``Simone\,\Vtextvisiblespace[1em]\,mangiò\,\Vtextvisiblespace[1em]\,l'\,\Vtextvisiblespace[1em]\,insalata''\\
			\alert{F2:} ``L'\,\Vtextvisiblespace[1em]\,insalata\,\Vtextvisiblespace[1em]\,mangiò\,\Vtextvisiblespace[1em]\,Simone''\\\\
		{\tiny
		\begin{table}
			\setlength{\tabcolsep}{4pt}
			\renewcommand{\arraystretch}{1}
			\hskip-3.5cm\begin{tabular}{cc|c|c|c}
		...
		\begin{minipage}[t]{.75\textwidth}
			...
			\setlength{\beforeepigraphskip}{5pt}
			\setlength{\epigraphwidth}{0.9\textwidth}
			\epigraph{\textit{\alert{F1:} ``Concetti come <<Re>> e <<Regina>> dovrebbero essere correlati\ldots''\\
			\alert{F2:} ``Sinonimi come <<felice>> e <<gioioso>> dovrebbero avere una rappresentazione simile\ldots''\\
			\alert{F3:} ``E con parole come <<pitone>> e <<serpente>>, una generalizzazione dell'altra?!''}}{\textbf{Mancanza di relazioni semantiche}}
		\end{minipage}
		}
		\only<3|handout:3>{
		\begin{minipage}[t]{.75\textwidth}
...
\begin{frame}[t,fragile] \frametitle{Word2Vec: Come Funziona}
	...
	{\small
		\begin{itemize}[leftmargin=10pt,align=right]
			\onslide<1->{\item[\alert{\faHandORight}] \alert{Principio fondamentale:} parole che appaiono in contesti simili tendono ad avere significati simili}
			\onslide<2->{\item[\alert{\faHandORight}] \alert{Addestramento:} predire se due parole sono vicine in una frase}
			\onslide<3->{\item[\alert{\faHandORight}] \alert{Risultato:} parole con significati simili hanno \textit{embeddings} vicini nello spazio}
		\end{itemize}
		\vspace*{.3cm}
		\only<1|handout:1>{
		\begin{minipage}[t]{\textwidth}
			...
			Esempi di contesti:\\
			"Il mio \_\_\_ dorme nella cuccia" -> probabilmente "cane", "gatto", \ldots\\
			"Ho portato il mio \_\_\_ dal veterinario" -> "cane", "gatto", "pappagallo", \ldots\\
			"Il \_\_\_ gioca in giardino" -> "bambino", "cane", \ldots}}{\textbf{Principio alla base di Word2Vec}}
		\end{minipage}
		}
		\only<2|handout:2>{
		\begin{minipage}[t]{\textwidth}
			...
			\alertedcircled{1} Assegna a ogni parola un vettore casuale (dimensione fissa)\\
			\alertedcircled{2} Prendi coppie di parole dal testo\\
			\alertedcircled{3} La rete predice: sono vicine in una frase?\\
			\alertedcircled{4} Aggiorna i vettori in base alla correttezza}}{\textbf{Training di Word2Vec}}
		\end{minipage}
		}
		\only<3|handout:3>{
		\begin{minipage}[t]{\textwidth}
