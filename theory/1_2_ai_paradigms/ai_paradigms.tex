...
\begin{frame}[t,fragile] \frametitle{AI debole: esempi}
	\framesubtitle{Casi d'uso}
		...
		\end{itemize}
		\vspace*{.3cm}
		\begin{block}{Analizzatore \textit{sentiment} (\textcopyright\ Claude)}
        	\begin{minted}{java}
    @RestController
...
\begin{frame}[t,fragile] \frametitle{AI debole: esempi}
	\framesubtitle{Casi d'uso}
		...
		\end{itemize}
		\vspace*{.3cm}
    	\begin{block}{\textit{Chatbot} tradizionale con regole fisse (\textcopyright\ Claude)}
        	\begin{minted}{java}
    @Component
...
\begin{frame}[t,fragile] \frametitle{AI debole: esempi}
	\framesubtitle{Casi d'uso}
		...
		\end{itemize}
		\vspace*{.3cm}
    	\begin{block}{Sistema di rilevamento frodi basato su ML (\textcopyright\ Claude)}
        	\begin{minted}{java}
    @Service
...
\begin{frame}[t,fragile] \frametitle{AI Forte: Esempi Ipotetici}
	\framesubtitle{Cosa potrebbe fare una GAI}
	{\small
		\begin{itemize}[leftmargin=10pt,align=right]
			...
			\begin{itemize}[leftmargin=10pt,align=right]
				\item[\alert{\faArrowCircleRight}] \textit{Explainable} AI
			\end{itemize}
		\end{itemize}
		\vspace*{.3cm}
		\hspace*{4cm}
		...
		\begin{minipage}[t]{\textwidth}
			\begin{minipage}[t]{0.6\textwidth}
				...
				\epigraph{\textit{Analizzerò il tuo sistema legacy in COBOL, lo convertirò in microservizi Spring Boot, ottimizzerò le performance del database, implementerò la sicurezza OAuth2 e creerò una UI React responsive. Tutto completato in 30 minuti.}}{\textbf{Scenario ipotetico di GAI programmatore}}
			\end{minipage}
		\end{minipage}
		}
		\only<2|handout:2>{
		...
		\begin{minipage}[t]{\textwidth}
			\begin{minipage}[t]{0.6\textwidth}
				...
				\setlength{\beforeepigraphskip}{5pt}
				\setlength{\epigraphwidth}{0.9\textwidth}
				\epigraph{\textit{Non ho mai visto il framework Quantum-Spring che avete appena rilasciato, ma dopo aver letto 3 esempi di codice ho compreso i pattern architetturali e posso già ottimizzare la vostra implementazione.}}{\textbf{Apprendimento rapido da pochi esempi}}
			\end{minipage}
		\end{minipage}
		}
		\only<4|handout:4>{
		\begin{minipage}[t]{\textwidth}
			\begin{minipage}[t]{0.6\textwidth}
				...
				\setlength{\beforeepigraphskip}{5pt}
				\setlength{\epigraphwidth}{0.9\textwidth}
				\epigraph{\textit{Il bug nel vostro sistema non è dove pensate. Il vero problema è una race condition causato da un pattern di design che crea dipendenze cicliche tra tre microservizi, amplificato dal garbage collector di JVM sotto carico elevato.}}{\textbf{Ragionamento causale profondo}}
			\end{minipage}
		\end{minipage}
		}
	}
