...
\begin{frame}[t,fragile] \frametitle{Artificial Intelligence}
	...
	{\scriptsize
	    \begin{minipage}[t]{\textwidth}
			\begin{minipage}[t]{0.6\textwidth}
	    		\begin{itemize}[leftmargin=10pt,align=right]
					\onslide<1->{\item[\alert{\faHandORight}] \alert{AI Debole (Narrow AI):} sistemi specializzati per compiti specifici
					\begin{itemize}[leftmargin=10pt,align=right]
						\item[\alert{\faHandORight}] Contesto \alert{noto a priori}, definito dai dati su cui il sistema è addestrato
						\item[\alert{\faHandORight}] Incapaci di generalizzare oltre il loro scopo
					\end{itemize}}
					\onslide<2->{\item[\alert{\faHandORight}] \alert{AI Forte (General AI):} sistemi autonomi e coscienti in grado di affrontare situazioni anche impreviste 
					\begin{itemize}[leftmargin=10pt,align=right]
						\item[\alert{\faHandORight}] Contesto \alert{dinamico}, definito dalla realtà, interpretato dal sistema
						\item[\alert{\faHandORight}] Soluzione trovata a fronte di una \alert{comprensione della natura del problema}
						\item[\alert{\faHandORight}] Creatività e intuizione
					\end{itemize}}					
					\onslide<3->{\item[\alert{\faHandORight}] \alert{Super AI:} intelligenza che supera quella umana in tutti i domini
					\begin{itemize}[leftmargin=10pt,align=right]
						\item[\alert{\faHandORight}] Teoria dell'Auton
					\end{itemize}}					
					\onslide<4->{\item[\alert{\faExclamationTriangle}] Oggi esistono \alert{solo} sistemi di AI Debole}
					\begin{itemize}[leftmargin=10pt,align=right]
						\onslide<5->{\item[\alert{\faHandORight}] Stime GAI: 2040--2060}
					\end{itemize}	
				\end{itemize}
            \end{minipage}
...
\begin{frame}[t,fragile] \frametitle{AI debole: esempi}
	\framesubtitle{Casi d'uso}
		\begin{itemize}[leftmargin=10pt,align=right]
			\item[\alert{\faHandORight}] \alert{Analisi del \textit{sentiment}:} classificare recensioni dei clienti
		\end{itemize}
		\vspace*{.3cm}
...
\begin{frame}[t,fragile] \frametitle{AI debole: esempi}
	\framesubtitle{Casi d'uso}
		\begin{itemize}[leftmargin=10pt,align=right]
			\item[\alert{\faHandORight}] \alert{\textit{Chatbot} per supporto:} rispondere a domande frequenti
		\end{itemize}
		\vspace*{.3cm}
...
\begin{frame}[t,fragile] \frametitle{AI debole: esempi}
	\framesubtitle{Casi d'uso}
		\begin{itemize}[leftmargin=10pt,align=right]
			\item[\alert{\faHandORight}] \alert{Rilevamento frodi:} identificare transazioni sospette
		\end{itemize}
		\vspace*{.3cm}
...
\begin{frame}[t,fragile] \frametitle{AI Forte: Esempi Ipotetici}
	...
	{\small
		\begin{itemize}[leftmargin=10pt,align=right]
			\onslide<1->{\item[\alert{\faHandORight}] \alert{Programmatore universale:} scrivere codice in qualsiasi linguaggio e dominio}
			\onslide<2->{\item[\alert{\faHandORight}] \alert{\textit{Problem solver} generale:} risolvere problemi mai visti prima}
			\onslide<3->{\item[\alert{\faHandORight}] \alert{Apprendimento rapido:} imparare nuovi concetti da pochi esempi}
			\onslide<4->{\item[\alert{\faHandORight}] \alert{Ragionamento causale:} comprendere cause ed effetti complessi}
		\end{itemize}
		\vspace*{.3cm}
		\hspace*{4cm}
		\only<1|handout:1>{
