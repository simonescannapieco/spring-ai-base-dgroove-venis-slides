...
\begin{frame}[t,fragile] \frametitle{Cronistoria della AI}
	...
	\only<5>{\item[\alert{\faHandORight}] \alert{Paradosso di Moravec:} più il compito è semplice per un umano, più è difficile per una macchina da emulare (percezione, mobilità); più è facile per una macchina, più è difficile per un umano (calcolo veloce)}
	\end{itemize}
	\only<1>{
	\begin{minipage}[t]{\textwidth}
		\begin{minipage}[t]{\textwidth}
	...
	\begin{minipage}[t]{\textwidth}
		...
		\begin{minipage}[t]{0.6\textwidth}
			\begin{itemize}[leftmargin=10pt,align=right]
			\item[\alert{\faHandORight}] Finanziato dalla Marina statunitense
			\item[\alert{\faHandORight}] Dimensioni unità centrale (H $\times$ L $\times$ P): \alert{167 $\times$ 176 $\times$ 80 cm}
			\item[\alert{\faHandORight}] Peso: \alert{5 Ton circa}
			\item[\alert{\faHandORight}] Investimento dichiarato per ingegnerizzazione (al 2025): \alert{1.2 Milioni USD circa}
		\end{itemize}
		\end{minipage}
	\end{minipage}
	}
