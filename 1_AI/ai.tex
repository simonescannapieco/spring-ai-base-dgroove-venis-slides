...
\begin{frame}[t,fragile] \frametitle{Cronistoria della AI}
	...
	\only<5>{\item[\alert{\faHandORight}] \alert{Paradosso di Moravec (1988):} più il compito è semplice per un umano, più è difficile per una macchina da emulare (e viceversa)}
	\only<6>{\item[\alert{\faHandORight}] \alert{Crollo dei sistemi esperti:} \normalcircled{1} troppo costosi da mantenere, \normalcircled{2} difficili da aggiornare, \normalcircled{3} non robusti di fronte a \textit{input} insoliti}
	\end{itemize}
	\vspace*{.3cm}
	\only<1>{
	\begin{minipage}[t]{\textwidth}
